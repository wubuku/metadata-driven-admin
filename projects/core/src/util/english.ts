...
export class RegExpRule implements Rule {
  ...
  public getPlural(singular: string): string | null {
    ...
    if (this.singular.test(singular)) {
      return singular.replace(this.singular, this.plural);
    } else {
      return null;
    }
  }
...
export class English extends TwoFormInflector {
   ...
   */
  static pluralByCount(word: string, count: number) {
   ...
   * @return form of the word correct for given count
   */
  getPluralByCount(word: string, count: number) {
    if (count === 1) {
      return word;
    }
